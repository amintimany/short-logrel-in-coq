...
| IVFold e : is_val e → is_val (Fold e).
...
| TPFold Γ e τ : typed Γ e τ.[TRec τ/] → typed Γ (Fold e) (TRec τ)
| TPUnfold Γ e τ : typed Γ e (TRec τ) → typed Γ (Unfold e) τ.[TRec τ/].
...
| STUnfold e : is_val e → head_step (Unfold (Fold e)) e.
...
| ECTX_Fold K : is_ectx K → is_ectx (λ e, Fold (K e))
| ECTX_Unfold K : is_ectx K → is_ectx (λ e, Unfold (K e)).
...
Proof.
  ...
  - left; constructor.
  - left; constructor.
  - destruct IHe; [left; constructor; assumption|right; inversion 1; subst; tauto].
Qed.
...
Proof.
  ...
  - inversion HK;inversion 1; subst; right; constructor.
  - inversion HK;inversion 1; subst; right; constructor.
  - inversion HK;inversion 1; subst; right; [constructor; assumption|eapply is_ectx_val; eauto].
Qed.
  ...
  - destruct HK';
    ...
    destruct (IHK e e' _ HK') as [[K'' [? HK'']]|[K'' [? HK'']]];
      [congruence|assumption|assumption| left|right];
        (exists K''; split; [assumption|intros ?; rewrite HK''; trivial]).
  - destruct HK';
      [left; exists (λ e, Fold (K e)); auto with is_ectx; fail|
        congruence|congruence| congruence|congruence|congruence|congruence|congruence| |congruence].
    simplify_eq Hee; intros; subst.
    destruct (IHK e e' _ HK') as [[K'' [? HK'']]|[K'' [? HK'']]];
      [congruence|assumption|assumption| left|right];
        (exists K''; split; [assumption|intros ?; rewrite HK''; trivial]).
  - destruct HK';
      [left; exists (λ e, Unfold (K e)); auto with is_ectx; fail|
        congruence|congruence| congruence|congruence|congruence|congruence|congruence|congruence|].
    simplify_eq Hee; intros; subst.
    destruct (IHK e e' _ HK') as [[K'' [? HK'']]|[K'' [? HK'']]];
      [congruence|assumption|assumption| left|right];
...
Lemma det_head_step_unfold e : is_val e → det_head_step (Unfold (Fold e)) e.
Proof.
  intros; split; [econstructor; assumption| inversion 1; subst; reflexivity].
Qed.
...
Ltac solve_det_head_step :=
  solve [eapply det_head_step_fst; eassumption|
          ...
          eapply det_head_step_unfold; eassumption].
